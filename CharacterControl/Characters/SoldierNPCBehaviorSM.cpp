		...
		void SoldierNPCBehaviorSM::do_SoldierNPCMovementSM_Event_TARGET_REACHED(PE::Events::Event* pEvt)
		{
			...
			if (m_state == PATROLLING_WAYPOINTS)
			{
				...
				if (pGameObjectManagerAddon)
				{
					...
					char *nextWPName = "";
					if (randnum == 0) {
						nextWPName = pWP->m_nextPossibleWayPoint0;
					}
					else if (randnum == 1) {
						nextWPName = pWP->m_nextPossibleWayPoint1;
					}
					else if (randnum == 2) {
						nextWPName = pWP->m_nextPossibleWayPoint2;
					}
					if (pWP && StringOps::length(nextWPName) > 0)
					{
						// have next waypoint to go to
						pWP = pGameObjectManagerAddon->getWayPoint(nextWPName);
						if (pWP)
						{
